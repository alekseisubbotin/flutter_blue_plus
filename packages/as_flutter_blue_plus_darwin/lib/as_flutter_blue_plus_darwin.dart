...
import 'package:as_flutter_blue_plus_platform_interface/as_flutter_blue_plus_platform_interface.dart';

final class FlutterBluePlusDarwin extends FlutterBluePlusPlatform {
  final _methodChannel = const MethodChannel('flutter_blue_plus/methods');
  ...
  Future<T?> _callDarwinMethod<T>(
    ...
    // invoke
    final out = await _methodChannel.invokeMethod<T>(method, arguments);
  ...
  Future<void> _methodCallHandler(
    ...
    switch (call.method) {
      ...
      case 'OnScanResponse':
        print('AAAAAAAA ${call.arguments}');
        final map = call.arguments is Map ? call.arguments as Map : null;
        if (map == null) {
          _onScanResponseController.add(
            BmScanResponse(
              ...
              success: false,
              errorCode: 500,
              errorString: 'internal error 1 communicating with ios platform',
            ),
          );
          return;
        }
        final json = Map<String, dynamic>.from(map);
        final id = json['remote_id'];
        final rssi = json['rssi'];
        final advertisementData = json['advertisement_data'];
        if (advertisementData is! Map) {
          _onScanResponseController.add(
            BmScanResponse(
              ...
              success: false,
              errorCode: 500,
              errorString: 'internal error 2 communicating with ios platform',
            ),
          );
        }
